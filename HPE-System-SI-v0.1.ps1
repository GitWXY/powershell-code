...
#
#[PURPOSE]:
#定时检测本地服务器【Starchase_Interface】是否有新文件产生
#如有新文件产生则将目录剪切到指定目录中【E:\StarchaseBAK】下
#
...
if(Test-Path $cfgfilepath) {
	...
	$serverfilepath = "\\"+$hostname+"\Starchase_Interface"
		LogInfo "Start to detect new files in $($serverfilepath)"
		#foreach ($filename in $filenameArray) {
		$folderCount = Get-ChildItem -Path \\DCFIleServer\Starchase_Interface\ -Force | Where-Object { $_.PSIsContainer -eq $true } | Measure-Object | Select-Object -ExpandProperty Count
		if($folderCount -eq 0) {
